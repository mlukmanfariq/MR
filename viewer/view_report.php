				...
				<ul class="nav nav-list">
					...
					<li class="active">
						...
						<ul class="submenu">
							...
							<?php
								$query = pg_query($conn, "SELECT * FROM tbl_r_employee WHERE nik = '$nik' ");
								$result = pg_fetch_array($query);				
								$section =  $result['section'];
								if ($section == 'Sparepart') {
									echo '<li class="">
											<a href="view_all_mr.php">
												<i class="menu-icon fa fa-list-ol"></i>View All Item
											</a>
											<b class="arrow"></b>
										</li>';
								}
								...
								<?php
									...
									<div class="box-body table-responsive">
										<table id="dynamic-table_pending" class="table table-bordered table-hover " cellspacing="0" width="100%">
											...
											<tbody>';
												$no = 1;
												$query = pg_query($conn, "SELECT user_name, created_date, mr_no, pr_no, purpose, header_id, trim(status_desc) as status, coalesce(remarks,'-') as remarks FROM public.vw_t_document_last_history WHERE requestor_id = $id_code order by modified_date_app desc; ");
