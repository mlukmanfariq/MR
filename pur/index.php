					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<?php
									$nik = $_SESSION['user_pur'];
									$query = pg_query($conn, "SELECT count(header_id) as notif FROM public.vw_t_approvaldocument where nik_approval = 'pur'");
									...
									$query2 = pg_query($conn, "SELECT count(header_id) as notif FROM public.vw_t_approvaldocument where last_step_dokumen = 5 and coalesce(requisition_header_id,0) = 0");
									while($data = pg_fetch_array($query2)){
										echo '<div class="col-lg-3 col-md-6">';
											echo '<div class="panel panel-warning">';
												echo '<div class="panel-heading">';											
													echo '<div class="row">';
														...
														echo '<div class="col-xs-9 text-right">';
															echo '<div class="huge">'.$data['notif'].'</div>';
															echo '<div>Pending Sync Oracle!</div>';
														echo '</div>';
													echo '</div>';
												echo '</div>';
												echo '<a href="sync.php">';
													echo '<div class="panel-footer">';
														echo '<span class="pull-left">View Details</span>';
														echo '<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>';
														echo '<div class="clearfix"></div>';
									...
									$query1 = pg_query($conn, "SELECT count(header_id) as notif FROM public.vw_t_approvaldocument where last_step_dokumen = 5 and coalesce(pr_no,'') = '' and coalesce(requisition_header_id,0) > 0");
									while($data = pg_fetch_array($query1)){
										echo '<div class="col-lg-3 col-md-6">';
									...
									if ((pg_num_rows($query) == 0) && (pg_num_rows($query1) == 0) && (pg_num_rows($query2) == 0)){
								...
								<?php
									if(isset($_GET['last_step_dokumen'])){
										...
										if ($last_step_dokumen == 0) {
											...
											echo'<div class="table-header">
												Table List Pending PR No. in Oracle
											</div>';
										}
										...
										<div class="box-body table-responsive">
											<table id="dynamic-table_detail" class="table table-bordered table-hover " cellspacing="0" width="100%">
												...
												<tbody>';
													$no = 1;
													if ($last_step_dokumen == 0) {
														$query = pg_query($conn, "SELECT created_date, user_department, user_name, header_id, mr_no, suggested_vendor, header_id, trim(step_description) as status FROM public.vw_t_approvaldocument where nik_approval = 'pur' order by created_date desc, created_by; ");										
													} else if ($last_step_dokumen == 1) {
														$query = pg_query($conn, "SELECT created_date, user_department, user_name, header_id, mr_no, suggested_vendor, header_id, trim(step_description) as status FROM public.vw_t_approvaldocument where last_step_dokumen = 5 and coalesce(pr_no,'') = '' and coalesce(requisition_header_id,0) > 0 order by created_date desc, created_by; ");										
													}													
													while ($result = pg_fetch_array($query)){ echo' 
														<tr>	
															...
															<td>'.$result['status'].'</td>';
															if ($last_step_dokumen == 0) {
																echo'<td><center><a href="update_form_request.php?header_id='.$result['header_id'].'"><input type="submit" value="Action" name="finish" class="btn btn-warning btn-xs"/></a></td>';														
															} else if ($last_step_dokumen == 1) {
