					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<?php
									$nik = $_SESSION['user'];
									$query = pg_query($conn, "SELECT count(created_by) as notif, user_department FROM public.tbl_t_request_header where (last_step_dokumen = 0 or (last_step_dokumen > 90 and last_step_dokumen < 100)) and created_by = '$nik' group by user_department");
									...
									$query1 = pg_query($conn, "SELECT count(created_by) as notif, user_department FROM public.tbl_t_request_header where (last_step_dokumen > 0 AND last_step_dokumen < 5) and created_by = '{$_SESSION['user']}' group by user_department");
										...
										echo '<div class="col-lg-3 col-md-6">';
											echo '<div class="panel panel-warning">';
												echo '<div class="panel-heading">';											
													...
													echo '</div>';
												echo '</div>';
												echo '<a href="index.php?last_step_dokumen=1">';
													echo '<div class="panel-footer">';
														echo '<span class="pull-left">View Details</span>';
														echo '<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>';
														echo '<div class="clearfix"></div>';
									...
									$query3 = pg_query($conn, "SELECT count(created_by) as notif, user_department FROM public.tbl_t_request_header where last_step_dokumen = 5 and coalesce(pr_no,'') = '' and created_by = '{$_SESSION['user']}' group by user_department");
												...
												echo '<div class="panel-heading">';											
													echo '<div class="row">';
														...
														echo '<div class="col-xs-9 text-right">';
															...
															echo '<div>Waiting No. PR!</div>';
														echo '</div>';
													echo '</div>';
												echo '</div>';
												echo '<a href="index.php?last_step_dokumen=5">';
													echo '<div class="panel-footer">';
														echo '<span class="pull-left">View Details</span>';
														echo '<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>';
														echo '<div class="clearfix"></div>';
									...
									$query4 = pg_query($conn, "SELECT count(created_by) as notif, user_department FROM public.tbl_t_request_header where last_step_dokumen = 5 and coalesce(pr_no,'') > '' and created_by = '{$_SESSION['user']}' group by user_department");
										...
										echo '<div class="col-lg-3 col-md-6">';
											echo '<div class="panel panel-success">';
												echo '<div class="panel-heading">';											
													...
													echo '</div>';
												echo '</div>';
												echo '<a href="index.php?last_step_dokumen=6">';
													echo '<div class="panel-footer">';
														echo '<span class="pull-left">View Details</span>';
														echo '<span class="pull-right"><i class="fa fa-arrow-circle-right"></i></span>';
														echo '<div class="clearfix"></div>';
									...
									if ((pg_num_rows($query) == 0) && (pg_num_rows($query3) == 0) && (pg_num_rows($query1) == 0) && (pg_num_rows($query4) == 0)){
							...
							<div class="row">
								<?php
									if(isset($_GET['last_step_dokumen'])){
										...
										} else if ($last_step_dokumen == 1) {
											echo'<div class="table-header">
												Table List Document Waiting Superior Approval
											</div>';
										} else if ($last_step_dokumen == 5) {
											echo'<div class="table-header">
												Table List Document Waiting PR No.
											</div>';
										} else if ($last_step_dokumen == 6) {
											echo'<div class="table-header">
										...
										<div class="box-body table-responsive">
											<table id="dynamic-table_detail" class="table table-bordered table-hover " cellspacing="0" width="100%">
												...
												<tbody>';
													$no = 1;
													if ($last_step_dokumen == 0) {
														$query = pg_query($conn, "SELECT created_date, mr_no, purpose, header_id, trim(status_desc) as status, coalesce(remarks,'-') as remarks FROM public.vw_t_document_last_history WHERE (last_step_dokumen = 0 or (last_step_dokumen > 90 and last_step_dokumen < 100)) and created_by = '{$_SESSION['user']}' order by created_date desc; ");
													} else if (($last_step_dokumen == 1)) {
														$query = pg_query($conn, "SELECT created_date, mr_no, purpose, header_id, trim(status_desc) as status, coalesce(remarks,'-') as remarks FROM public.vw_t_document_last_history WHERE (last_step_dokumen > 0 and last_step_dokumen < 5) and created_by = '{$_SESSION['user']}' order by created_date desc; ");
													} else if ($last_step_dokumen == 5) {
														$query = pg_query($conn, "SELECT created_date, mr_no, purpose, header_id, trim(status_desc) as status, coalesce(remarks,'-') as remarks FROM public.vw_t_document_last_history WHERE last_step_dokumen = 5 and coalesce(pr_no,'') = '' and created_by = '{$_SESSION['user']}' order by created_date desc; ");
													} else if ($last_step_dokumen == 6) {
														$query = pg_query($conn, "SELECT created_date, mr_no, purpose, header_id, trim(status_desc) as status, coalesce(remarks,'-') as remarks FROM public.vw_t_document_last_history WHERE last_step_dokumen = 5 and coalesce(pr_no,'') > '' and created_by = '{$_SESSION['user']}' order by created_date desc; ");
													}												
