...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="../assets/css/load/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous" />
		<link rel="stylesheet" href="../assets/css/load/bootstrap-theme.min.css" integrity="sha384-rHyoN1iRsVXV4nD0JutlnGaslCJuC7uwjduW9SVrLvRYooPp2bWYgmgJQIXwl/Sp" crossorigin="anonymous" />
		<script src="../assets/js/load/jquery.min.js"></script> 
		<script src="../assets/js/load/bootstrap.min.js" integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa" crossorigin="anonymous"></script>
	...
	<body class="no-skin">
				...
				<div class="main-content-inner">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Update Form Requisition</li>
						</ul><!-- /.breadcrumb -->
					</div>
					<div class="page-content">
						<div class="page-header">
							<h1>
								Update Form Requisition
							</h1>
						</div><!-- /.page-header -->
						<div class="row">
							<div class="col-xs-12">
								<div class="panel panel-primary">
									...
									<!--<form class="form-horizontal style-form" action="" method="get"> -->
										<div class="panel-body">
											<div class="form-group">
												<?php										
													$query = pg_query($conn, "SELECT header_id, mr_no, user_section, user_department, cost_center, trim(suggested_vendor) as suggested_vendor, pr_no, requestor_id, 
													created_date, created_by, modified_date, modified_by, last_step_dokumen, trim(purpose) as purpose, requisition_header_id FROM tbl_t_request_header 
												...
												<table cellpadding="6" class="table table-bordered">
													<tr>
														...
														<td>:</td>
														<td>
															<textarea rows="2" cols="40" class="k-textbox textbox-custom" id="txt_suggestedVendor" name="txt_suggestedVendor" maxlength="239"><?php echo $view['suggested_vendor'];?></textarea>
														</td>
													...
													<tr>
														...
														<td>:</td>
														<td>
															<textarea rows="2" cols="40" class="k-textbox textbox-custom" id="txt_purpose" name="txt_purpose" maxlength="239"><?php echo $view['purpose'];?></textarea>
														</td>
													...
													<tr>
														...
														<td>:</td>
														<td>
															<input class="k-textbox textbox-custom" id="txt_NoMR" name="txt_NoMR" value="<?php echo $view['mr_no'];?>" placeholder="Generate by System" readonly/>
														</td>
													...
													<div class="box-body table-responsive">
														<table id="dynamic-table_detail" class="table table-bordered table-hover " cellspacing="0" width="100%">
															...
															<tbody>';
																if($cek_row > 0){
																	$no = 1;
																	$query = pg_query($conn, "SELECT *, replace(replace(desc_material,'|*|',''''),'|and|','&') as a FROM vw_t_requestall where header_id = {$header_id}  and created_by = '{$user}' ORDER BY row_detail_id");
									...
									<!--</form> -->
										...
										<img id="loading" src="../assets/img/pgb1.gif" /> <!-- Loading Image -->
										</br>
										</br>
										&nbsp; &nbsp; &nbsp;
										<button name="clear" id="cancel" class="btn btn-default btn-sm" onclick="cancelInsert()">Clear</button>								
										<button type="submit" id="submitDraft" name ="save" class="btn btn-warning btn-sm" onclick="header('draft')">Save as Draft</button>
							...
							<div class="panel-heading">Purchase Requisition Detail</div>
								...
								<table cellpadding="6" class="table table-bordered">
									<tr>
										...
										<td>:</td>
										<td>
											<input class="k-textbox textbox-custom" id="txt_itemNo" name="txt_itemNo" placeholder="Searching Description" readonly/>
										</td>

										<td>UOM</td>
										<td>:</td>
										<td>
											<input class="k-textbox textbox-custom" id="txt_UOM" name="txt_UOM" placeholder="Searching Description" readonly/>
										</td>
									</tr>
									<tr>
										<td>Description of Material</td>
										<td>:</td>
										<td class="control-group danger">
											<input class="k-textbox textbox-custom" id="txt_Description" name="txt_Description" placeholder="Search Here" maxlength="250"/>											
									...
									<tr>																		
										...
										<td>:</td>
										<td>
											<input type="number" id="txt_stockBal" name="txt_stockBal" class="k-textbox textbox-custom" placeholder="Searching Description" title="Searching Description" readonly/>
										</td>	
									</tr>
									<tr>
										<td>Min. Stock</td>
										<td>:</td>
										<td>
											<input type="number" id="txt_minStock" name="txt_minStock" class="k-textbox textbox-custom" placeholder="Searching Description" title="Searching Description" readonly/>
										</td>

										<td>Max. Stock</td>
										<td>:</td>
										<td>
											<input type="number" id="txt_maxStock" name="txt_maxStock" class="k-textbox textbox-custom" placeholder="Searching Description" title="Searching Description" readonly/>
										</td>
									</tr>
									<tr>
										<td>No Capex</td>
								...
								&nbsp; &nbsp; 
								<button type="button" id="adddata" name ="adddata" class="btn btn-success btn-xs" onclick="header('addrow')"><span class="glyphicon glyphicon-saved"></span>&nbsp; Add Row</button>&nbsp; &nbsp; 
		...
		<script type="text/javascript">
			...
			function header(option){
				...
				var txt_suggestedVendor	= $("#txt_suggestedVendor").val().replace("'", "*").replace("&", ",and");
				var txt_NoPR 			= $("#txt_NoPR").val();
				var txt_purpose 		= $("#txt_purpose").val().replace("'", "*").replace("&", ",and");
				var txt_user			= $("#user").val();
				var txt_NoMR			= $("#txt_NoMR").val();
				var txt_date			= $("#txt_date").val();
				var txt_headerid		= $("#headerid").val();
				var txt_option			= option;
				...
				var dataString = 'user_section='+ txt_section + '&user_department='+ txt_Department + '&requestor_id='+ RequestorID + '&cost_center='+ txt_CostCenter 
								...
								+ '&option='+ txt_option  + '&mr_no='+ txt_NoMR  + '&created_date='+ txt_date + '&headerid='+ txt_headerid;
				console.log(dataString);
				if ((txt_Requestor != "") && (txt_purpose != "")){
					$.ajax({
						type: 'POST',
					...
					alert("Field Requestor or MR Desc still Empty!!!");
				}
			}
			...
			function addRow(){
				...
				var txt_DescriptionItem = $("#txt_DescriptionItem").val().replace("'", "*");
			...
			function editRow(){
				...
				table_id.onclick = function (event) {
					...
					var txt_nocapex			= document.getElementById('txt_nocapex_edit');
					...
					txt_maxStock.value = cells[8].innerHTML;
					txt_stockBal.value = cells[9].innerHTML;
					txt_nocapex.value = cells[10].innerHTML;
			...
			function cancelInsert(){
				var txt_option			= "clear";
				var txt_headerid		= $("#headerid").val();
				var txt_status_doc		= "101";
				var txt_status_desc		= "Clear Request by User";
				var txt_remarks			= "Clear Request by User";
			...
			function submitDraft(){
				...
				document.getElementById("submitDraft").disabled = true;
				...
				$.ajax({
					...
					success: function (result) {
						...
						document.getElementById("submitDraft").disabled = false;
