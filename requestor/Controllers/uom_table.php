<?php
	...
	$description1 = strtoupper($_REQUEST["description1"]);
	If (!$connOra) 
	{
		$e = oci_error();
		...
		echo '
			...
			<div class="box-body table-responsive">
				<table id="dynamic-table" class="table table-bordered table-striped table-hover">
					...
					<tbody>';
						$no = 1;
						$sqlOra="SELECT a.SEGMENT1, a.PRIMARY_UNIT_OF_MEASURE, REPLACE(REPLACE(a.DESCRIPTION, '&', '|and|'),'''','|*|') as DESCRIPTION, b.CATEGORY_ID, 
						...
						a.ORGANIZATION_ID = b.ORGANIZATION_ID and a.ORGANIZATION_ID IN (43,44,45) AND a.INVENTORY_ITEM_STATUS_CODE <> 'Inactive'
						AND (upper(REPLACE(a.DESCRIPTION, '''', '*')) like '%".$description1."%' OR upper(REPLACE(a.SEGMENT1, '''', '*')) = '".$description1."' ) AND ROWNUM <= 10 ORDER BY a.SEGMENT1";					
