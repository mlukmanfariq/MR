				...
				<div class="main-content-inner">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li>
								<i class="ace-icon fa fa-refresh"></i>&nbsp;Sync Oracle
							</li>
						</ul><!-- /.breadcrumb -->
					</div>
									...
									<div class="panel-body">
										<div class="form-group">																					
											<?php
												...
												{	
													$no = 1;
													$query = pg_query($conn, "SELECT header_id FROM public.tbl_t_request_header where coalesce(requisition_header_id,0) > 0 and coalesce(pr_no,'') = '' order by created_date desc");																							
													$cek_row = pg_num_rows($query);
													if ($cek_row > 0){
														while ($result = pg_fetch_array($query)){
															if ($cek_row == 1) {
																$list_reqID = "'".$result['header_id'] . "'";
															} else {
																if ($no == 1) {
																	$list_reqID = "'".$result['header_id'] . "','";
																} else if ($no < $cek_row){
																	$list_reqID = $list_reqID . $result['header_id'] . "','";
																}else {
																	$list_reqID = $list_reqID . $result['header_id'] . "'";
																}		
															}																												
													...
													echo '
														...
														<div class="box-body table-responsive">
															<table id="dynamic-table_PR" class="table table-bordered table-striped table-hover">
																...
																<tbody>';
																	$no = 1;
																	$sqlOra="SELECT a.* FROM (SELECT INTERFACE_SOURCE_CODE, PREPARER_ID, SEGMENT1, DESCRIPTION, LAST_UPDATE_DATE FROM PO_REQUISITION_HEADERS_ALL ORDER BY LAST_UPDATE_DATE DESC) a 
