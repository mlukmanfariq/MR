					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<?php
									$nik = $_SESSION['admin'];
									$query = pg_query($conn, "SELECT count(header_id) as notif FROM public.vw_t_approvaldocument where (last_step_dokumen > 0 AND last_step_dokumen < 100) and coalesce(requisition_header_id,0) = 0;");
												...
												echo '<div class="panel-heading">';											
													echo '<div class="row">';
														...
														echo '<div class="col-xs-9 text-right">';
															...
															echo '<div>Outstanding Document!</div>';
														echo '</div>';
													echo '</div>';
												echo '</div>';
								...
								<?php
									if(isset($_GET['last_step_dokumen'])){
										...
										<div class="table-header">
												Table List Outstanding Document
										</div>
										<div class="box-body table-responsive">
											<table id="dynamic-table_detail" class="table table-bordered table-hover " cellspacing="0" width="100%">
												...
												<tbody>';
													$no = 1;
													$query = pg_query($conn, "SELECT created_date, user_department, user_name, mr_no, purpose, header_id, last_step_dokumen, trim(step_description) as status FROM public.vw_t_approvaldocument where (last_step_dokumen > 0 AND last_step_dokumen < 100) and coalesce(requisition_header_id,0) = 0 order by created_date desc; ");																						
													while ($result = pg_fetch_array($query)){ echo' 
														<tr>	
															...
															<td>'.$result['status'].'</td>';
															if ($result['last_step_dokumen'] < 5) {
																echo '<td><center><a href="appr_form_request.php?header_id='.$result['header_id'].'"><input type="submit" value="Action" name="finish" class="btn btn-warning btn-xs"/></a></td>';
															} else {
