						...
						<div class="row">
								...
								<?php
									...
									<div class="box-body table-responsive">
										<table id="dynamic-table_pending" class="table table-bordered table-hover " cellspacing="0" width="100%">
											...
											<tbody>';
												$no = 1;
												$query = pg_query($conn, "SELECT created_by, created_date, mr_no, pr_no, purpose, header_id, trim(status_desc) as status, coalesce(remarks,'-') as remarks, modified_date_app FROM public.vw_t_document_last_history WHERE created_by = '$nik' order by modified_date_app desc; ");
												while ($result = pg_fetch_array($query)){ echo' 
													<tr>	
														<td><center><span class="badge badge-info">'.$no.'</span></td>																		
														<td><span class="hidden">'.$result['modified_date_app'].'</span>'.date_convert($result['modified_date_app']).'</td>
							...
							<div class="col-xs-12">
								...
								<?php
									...
									echo '
									<div class="table-header">
										Table All History Document Approval
									</div>
									<div class="box-body table-responsive">
										<table id="dynamic-table_detail" class="table table-bordered table-hover " cellspacing="0" width="100%">
											...
											<tbody>';
												$no = 1;
												$query = pg_query($conn, "SELECT *, trim(status_desc) as status, coalesce(remarks,'-') as remarks
																		FROM public.vw_t_document_last_history 
																		WHERE mr_no IN (
																					SELECT distinct mr_no
																					FROM public.vw_t_document_history 
																					WHERE modified_by_app = '$nik'
																				) and (status_doc > 0) 
																		order by modified_date_app desc; ");
												while ($result = pg_fetch_array($query)){ echo' 
													<tr>	
														<td><center><span class="badge badge-info">'.$no.'</span></td>																		
