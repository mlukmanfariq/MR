	...
	<body class="no-skin">
							...
							<div class="panel-heading">Purchase Requisition Detail</div>
								...
								<table cellpadding="6" class="table table-bordered">
									...
									<tr>
										...
										<td>:</td>
										<td >
											<input class="k-textbox textbox-custom" id="txt_Description" name="txt_Description" placeholder="Search Here"/>											
									...
									<tr>
										...
										<td>
											With No Capex ? <br>
											<input id="yes_capex" type="radio" name="conf_capex" value="yes" onclick="javascript:yesnoCheck();" checked> Yes
											<input id="no_capex" type="radio" name="conf_capex" value="no" onclick="javascript:yesnoCheck();"> No
										</td>

										<td>Balance</td>
										<td>:</td>
										<td>
											<input class="k-textbox textbox-custom" id="txt_balance" name="txt_balance" maxlength="50"n readonly="" />	
										</td>
									</tr>
									<tr>
										<td></td>
										...
										<td>
											<!-- lkm -->
											<select name="txt_nocapex" id="txt_nocapex" name="txt_nocapex">
												<option value=0> --- Select No Capex --- </option>
												<?php 
													$query_dept = pg_query($conn, "SELECT departemen_id FROM tbl_r_departemen WHERE departemen_name = UPPER('$departemen_name') ");
													$rows = pg_fetch_array($query_dept);
													$dept_id = $rows['departemen_id'];
													
													$sql =  pg_query($conn, "SELECT capex_number FROM tbl_r_capex_dept WHERE departemen_id = $dept_id");
													while($row = pg_fetch_array($sql)){
															echo "<option value=\"{$row['capex_number']}\">{$row['capex_number']}</option>",PHP_EOL;
													}
												?>
											</select>
										</td>
										...
										<td>:</td>
										<td>
											<input class="k-textbox textbox-custom" type="number" id="txt_price" name="price" maxlength="50"/>	
										</td>
									</tr>
									</br>
								</table>
		...
		<script type="text/javascript">
			$(document).ready (function () {
				//get balance amount
				//lkm
				if (document.getElementById('yes_capex').checked==false){
					document.getElementById('txt_nocapex').disabled=true;
				}
				//lkm
				$('#txt_nocapex').on('change',function(){
			        var capex_number = $(this).val();
			        if(capex_number){
			            $.ajax({
			                type:'POST',
			                url:'Controllers/getBalance.php',
			                data:'capex='+capex_number,
			                success:function(result){
			                    $('#txt_balance').val(result);
			           		}
			            }); 
			        }
		    	});	
			...
			function header(option){
				...
				if (txt_Requestor != ""){
					//validator capex
					//lkm
					if (document.getElementById('yes_capex').checked==true){
						if ($('#txt_nocapex').val() == 0){
							alert('Please select No Capex');
						}
					}	
					//validator amount	
					//lkm
			...
			function addRow(){
				...
				var dept 				= '<?php echo $departemen_name?>';
				//var dataString = JSON.stringify(postData);
				//lkm
				var dataString = 'itemNo='+ txt_itemNo + '&description='+ txt_Description + '&reqtQty='+ txt_ReqtQty + '&uom='+ txt_UOM 
								...
								+ '&create_by='+ txt_user  + '&created_date='+ txt_date + '&headerid='+ txt_headerid + '&balance=' + txt_balance + '&price=' + txt_price + '&dept=' + dept;
				console.log(dataString);
				$.ajax({
					type: 'POST',
			...
			function validAmount(){
				...
				if (price > 0){
					var am_balance = balance - price;
					if ((am_balance < 0) && document.getElementById('yes_capex').checked==true){
						alert('Price is greater than balance, could not be continue');
						return false;
					}else{
						return true;
					}
				}else{
					alert('Invalid Data Price');
					return false;
				}
			...
			//lkm
			function yesnoCheck() {
			    if (document.getElementById('yes_capex').checked) {
			        document.getElementById('txt_nocapex').style.display = 'block';
			        document.getElementById('txt_nocapex').disabled=false;
			        $("#txt_nocapex").val(0);
			        $("#txt_balance").val(0);
			    }
			    else{
			    	document.getElementById('txt_nocapex').style.display = 'none';
			    	$("#txt_balance").val(0);
			    }
